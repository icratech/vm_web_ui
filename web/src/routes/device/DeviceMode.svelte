<script>
    ...
    let color_code = BASE.CHAR
    let color_code_txt_alpha = 0.79
    let color_code_txt = RGBA(color_code, color_code_txt_alpha)
    let color_code_bg = RGBA(color_code, 0.15)
    let color_code_border = RGBA(color_code, 0.25)
    let lbl = 'OFF'
    const findMode = ( ) => {
        // debug( "check mode", ) 
        lbl = 'OFF'
    ...
    let intervalID = setInterval( findMode, 500 )
    onDestroy( ( ) => { 
